...
// Function to insert labels in symbol table
void update_literal_data(lit_pool lp[10], lit_table lt[10], char *buffer1D){
	// Insert Label at first empty index
	int i;
	
	for(i=0;i<10;i++){
		if(lp[i].literal[0] == '\0'){
			strcpy(lp[i].literal,buffer1D);
			break;
		}
	}
...
// Insert address in Symbol Table
int insert_address_literal_data(lit_table lt[10], pool_t pt[10], int LC){
	int i,j,flg=0;
	
	for(j=0;j<10;j++){
		if(lt[j].address == -99 && lt[j].i != -99){
			if(flg==0){
				for(i=0;i<10;i++){
					if(pt[i].i == -99){
						pt[i].i = j;
...
void main(int argc, char *argv[]){
			...
			if(count == 0){
				...
				if(Op[0] == 1){
					...
					if(Op[0] != 1){
						...
						if(Op[1] == 2 || Op[1] == 5 && Op[0] == 3){
							LC =  insert_address_literal_data(lt,pt,LC);
						}
					}
					else{
			...
			// 2nd word in line
				...
				// AD class preceded : Process Operand
					...
					if(Op[1] == 1){
						...
						check_operand(Op, buffer1D);
						if(Op[0] == 5 || Op[0] == 6){
							printf("\n [ERROR] ORIGIN Assembler Directive cannot use REGISTERS or SYMBOLS :: LINE NUMBER %d ",num_lines);
						}
						else if(Op[0] == 7){
							LC = atoi(buffer1D);
							LC--;
						...
						check_operand(Op, buffer1D);
						if(Op[0] == 5 || Op[0] == 6){
							printf("\n [ERROR] EQU Assembler Directive cannot use REGISTERS or SYMBOLS :: LINE NUMBER %d ",num_lines);
						}
						else if(Op[0] == 7){
							// Write constant
							write_constant(write_IC, buffer1D, verbose);
			...
			// 3rd word in line
				...
				if(Op[0] == 2){
					...
					if(Op[0] == 5){
						...
						// Write literal
						update_literal_data(lp,lt,buffer1D);
					}
				}					
				else if(Op[0] == 7){
	...
	for(i=0;lp[i].literal[0];i++){
		fprintf(write_literal_file,"%s",lp[i].literal);
		fprintf(write_literal_file,"\n");
	}

	fprintf(write_literal_file,"\n\n");
	for(i=0;lt[i].literal[0];i++){
		fprintf(write_literal_file,"%d,",lt[i].i);
		fprintf(write_literal_file,"%s,",lt[i].literal);
		fprintf(write_literal_file,"%d",lt[i].address);		
		fprintf(write_literal_file,"\n");		
	}

	fprintf(write_literal_file,"\n\n");
	for(i=0;pt[i].i!=-99;i++){
		fprintf(write_literal_file,"%d",pt[i].i);
		fprintf(write_literal_file,"\n");
	}
